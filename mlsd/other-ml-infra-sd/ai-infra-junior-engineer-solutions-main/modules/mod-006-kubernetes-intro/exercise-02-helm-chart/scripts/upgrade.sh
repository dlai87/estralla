#!/bin/bash

################################################################################
# Helm Chart Upgrade Script
#
# This script upgrades an existing Flask application Helm release with support
# for different environments and rollback capabilities.
################################################################################

set -e

# Color definitions
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
RED='\033[0;31m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

# Default values
RELEASE_NAME="flask-app"
NAMESPACE="default"
ENVIRONMENT="dev"
CHART_DIR="../flask-app"
DRY_RUN=false
WAIT=true
TIMEOUT="5m"
ATOMIC=true
FORCE=false
RECREATE_PODS=false

# Helper functions
print_header() {
    echo ""
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}$1${NC}"
    echo -e "${BLUE}========================================${NC}"
}

print_success() {
    echo -e "${GREEN}✓ $1${NC}"
}

print_warning() {
    echo -e "${YELLOW}⚠ $1${NC}"
}

print_error() {
    echo -e "${RED}✗ $1${NC}"
}

print_info() {
    echo -e "${BLUE}ℹ $1${NC}"
}

show_usage() {
    cat << EOF
Usage: $0 [OPTIONS]

Upgrade existing Flask application Helm release

OPTIONS:
    -n, --name NAME          Release name (default: flask-app)
    -s, --namespace NAMESPACE Kubernetes namespace (default: default)
    -e, --environment ENV    Environment: dev, staging, prod (default: dev)
    -c, --chart PATH         Chart directory path (default: ../flask-app)
    -d, --dry-run            Perform a dry-run upgrade
    --no-wait                Don't wait for resources to be ready
    --no-atomic              Don't rollback on failure
    -f, --force              Force resource updates through delete/recreate
    --recreate-pods          Recreate pods during upgrade
    -t, --timeout DURATION   Timeout for upgrade (default: 5m)
    -h, --help               Show this help message

EXAMPLES:
    # Upgrade in development environment
    $0 -e dev

    # Upgrade in production with atomic rollback
    $0 -n my-app -e prod -s production

    # Dry-run upgrade
    $0 -e prod --dry-run

    # Force upgrade with pod recreation
    $0 -e prod --force --recreate-pods

EOF
}

# Parse command-line arguments
while [[ $# -gt 0 ]]; do
    case $1 in
        -n|--name)
            RELEASE_NAME="$2"
            shift 2
            ;;
        -s|--namespace)
            NAMESPACE="$2"
            shift 2
            ;;
        -e|--environment)
            ENVIRONMENT="$2"
            shift 2
            ;;
        -c|--chart)
            CHART_DIR="$2"
            shift 2
            ;;
        -d|--dry-run)
            DRY_RUN=true
            shift
            ;;
        --no-wait)
            WAIT=false
            shift
            ;;
        --no-atomic)
            ATOMIC=false
            shift
            ;;
        -f|--force)
            FORCE=true
            shift
            ;;
        --recreate-pods)
            RECREATE_PODS=true
            shift
            ;;
        -t|--timeout)
            TIMEOUT="$2"
            shift 2
            ;;
        -h|--help)
            show_usage
            exit 0
            ;;
        *)
            print_error "Unknown option: $1"
            show_usage
            exit 1
            ;;
    esac
done

# Validate environment
if [[ ! "$ENVIRONMENT" =~ ^(dev|staging|prod)$ ]]; then
    print_error "Invalid environment: $ENVIRONMENT. Must be dev, staging, or prod."
    exit 1
fi

# Navigate to script directory
cd "$(dirname "$0")"

################################################################################
# Prerequisites Check
################################################################################
print_header "Checking Prerequisites"

if ! command -v helm &> /dev/null; then
    print_error "Helm is not installed. Please install Helm first."
    exit 1
fi
print_success "Helm is installed ($(helm version --short))"

if ! command -v kubectl &> /dev/null; then
    print_error "kubectl is not installed. Please install kubectl first."
    exit 1
fi
print_success "kubectl is installed"

# Check cluster connectivity
if ! kubectl cluster-info &> /dev/null; then
    print_error "Cannot connect to Kubernetes cluster"
    exit 1
fi
print_success "Connected to Kubernetes cluster"

# Verify chart exists
if [ ! -d "$CHART_DIR" ]; then
    print_error "Chart directory not found: $CHART_DIR"
    exit 1
fi
print_success "Chart directory found: $CHART_DIR"

################################################################################
# Release Verification
################################################################################
print_header "Verifying Existing Release"

# Check if release exists
if ! helm list -n "$NAMESPACE" 2>/dev/null | grep -q "^$RELEASE_NAME"; then
    print_error "Release '$RELEASE_NAME' not found in namespace '$NAMESPACE'"
    print_info "Use install.sh script to install a new release"
    exit 1
fi
print_success "Release '$RELEASE_NAME' found"

# Show current release info
CURRENT_REVISION=$(helm list -n "$NAMESPACE" -o json | jq -r ".[] | select(.name==\"$RELEASE_NAME\") | .revision")
CURRENT_STATUS=$(helm list -n "$NAMESPACE" -o json | jq -r ".[] | select(.name==\"$RELEASE_NAME\") | .status")

print_info "Current revision: $CURRENT_REVISION"
print_info "Current status: $CURRENT_STATUS"

################################################################################
# Environment Configuration
################################################################################
print_header "Upgrade Configuration"

echo "Release Name:  $RELEASE_NAME"
echo "Namespace:     $NAMESPACE"
echo "Environment:   $ENVIRONMENT"
echo "Chart Path:    $CHART_DIR"
echo "Dry Run:       $DRY_RUN"
echo "Wait:          $WAIT"
echo "Atomic:        $ATOMIC"
echo "Force:         $FORCE"
echo "Recreate Pods: $RECREATE_PODS"
echo "Timeout:       $TIMEOUT"

# Determine values file
case $ENVIRONMENT in
    dev)
        VALUES_FILE="$CHART_DIR/values-dev.yaml"
        ;;
    staging)
        VALUES_FILE="$CHART_DIR/values-staging.yaml"
        if [ ! -f "$VALUES_FILE" ]; then
            print_warning "Staging values file not found, using production values"
            VALUES_FILE="$CHART_DIR/values-prod.yaml"
        fi
        ;;
    prod)
        VALUES_FILE="$CHART_DIR/values-prod.yaml"
        ;;
esac

if [ ! -f "$VALUES_FILE" ]; then
    print_error "Values file not found: $VALUES_FILE"
    exit 1
fi
print_success "Values file: $VALUES_FILE"

################################################################################
# Pre-upgrade Checks
################################################################################
print_header "Pre-upgrade Checks"

# Show what will change
print_info "Generating diff..."
if command -v helm-diff &> /dev/null; then
    helm diff upgrade "$RELEASE_NAME" "$CHART_DIR" \
        --namespace "$NAMESPACE" \
        --values "$VALUES_FILE" \
        --allow-unreleased || true
else
    print_warning "helm-diff plugin not installed. Skipping diff."
    print_info "Install with: helm plugin install https://github.com/databus23/helm-diff"
fi

# Check resource health
print_info "Checking current deployment health..."
if kubectl get deployment -n "$NAMESPACE" -l "app.kubernetes.io/instance=$RELEASE_NAME" &> /dev/null; then
    READY_PODS=$(kubectl get deployment -n "$NAMESPACE" -l "app.kubernetes.io/instance=$RELEASE_NAME" -o jsonpath='{.items[0].status.readyReplicas}' 2>/dev/null || echo "0")
    DESIRED_PODS=$(kubectl get deployment -n "$NAMESPACE" -l "app.kubernetes.io/instance=$RELEASE_NAME" -o jsonpath='{.items[0].status.replicas}' 2>/dev/null || echo "0")

    if [ "$READY_PODS" = "$DESIRED_PODS" ] && [ "$READY_PODS" != "0" ]; then
        print_success "Current deployment is healthy ($READY_PODS/$DESIRED_PODS pods ready)"
    else
        print_warning "Current deployment may have issues ($READY_PODS/$DESIRED_PODS pods ready)"
    fi
fi

################################################################################
# Update Dependencies
################################################################################
print_header "Updating Chart Dependencies"

if grep -q "dependencies:" "$CHART_DIR/Chart.yaml"; then
    print_info "Updating dependencies..."
    if helm dependency update "$CHART_DIR" 2>&1 | grep -v "WARNING"; then
        print_success "Dependencies updated"
    else
        print_warning "Could not update all dependencies (proceeding anyway)"
    fi
else
    print_info "No dependencies to update"
fi

################################################################################
# Validate Chart
################################################################################
print_header "Validating Chart"

print_info "Linting chart..."
if helm lint "$CHART_DIR" -f "$VALUES_FILE" > /dev/null 2>&1; then
    print_success "Chart validation passed"
else
    print_error "Chart validation failed"
    helm lint "$CHART_DIR" -f "$VALUES_FILE"
    exit 1
fi

################################################################################
# Security Checks
################################################################################
if [ "$ENVIRONMENT" = "prod" ]; then
    print_header "Production Security Checks"

    # Check for default secret key
    if grep -q "change-me-in-production" "$VALUES_FILE"; then
        print_error "Default secret key detected in production values file!"
        print_error "Please update flask.secretKey before deploying to production"
        exit 1
    fi
    print_success "No default secret keys found"

    # Require confirmation for production upgrades
    if [ "$DRY_RUN" = false ]; then
        echo ""
        print_warning "You are about to upgrade a PRODUCTION release!"
        read -p "Are you sure you want to continue? (yes/no) " -r
        echo
        if [[ ! $REPLY = "yes" ]]; then
            print_info "Upgrade cancelled"
            exit 0
        fi
    fi
fi

################################################################################
# Backup Current Release
################################################################################
if [ "$DRY_RUN" = false ]; then
    print_header "Backing Up Current Release"

    BACKUP_DIR="/tmp/helm-backup-$(date +%Y%m%d-%H%M%S)"
    mkdir -p "$BACKUP_DIR"

    print_info "Saving current values..."
    helm get values "$RELEASE_NAME" -n "$NAMESPACE" > "$BACKUP_DIR/values.yaml"

    print_info "Saving current manifest..."
    helm get manifest "$RELEASE_NAME" -n "$NAMESPACE" > "$BACKUP_DIR/manifest.yaml"

    print_success "Backup saved to: $BACKUP_DIR"
    print_info "To restore: helm rollback $RELEASE_NAME $CURRENT_REVISION -n $NAMESPACE"
fi

################################################################################
# Upgrade
################################################################################
print_header "Upgrading Helm Release"

# Build helm upgrade command
HELM_CMD="helm upgrade $RELEASE_NAME $CHART_DIR"
HELM_CMD="$HELM_CMD --namespace $NAMESPACE"
HELM_CMD="$HELM_CMD --values $VALUES_FILE"

if [ "$WAIT" = true ]; then
    HELM_CMD="$HELM_CMD --wait --timeout $TIMEOUT"
fi

if [ "$ATOMIC" = true ]; then
    HELM_CMD="$HELM_CMD --atomic"
fi

if [ "$FORCE" = true ]; then
    HELM_CMD="$HELM_CMD --force"
fi

if [ "$RECREATE_PODS" = true ]; then
    HELM_CMD="$HELM_CMD --recreate-pods"
fi

if [ "$DRY_RUN" = true ]; then
    HELM_CMD="$HELM_CMD --dry-run --debug"
fi

print_info "Executing: $HELM_CMD"
echo ""

if eval "$HELM_CMD"; then
    if [ "$DRY_RUN" = true ]; then
        print_success "Dry-run upgrade completed successfully"
    else
        print_success "Upgrade completed successfully"
    fi
else
    print_error "Upgrade failed"
    if [ "$ATOMIC" = true ] && [ "$DRY_RUN" = false ]; then
        print_info "Automatic rollback was performed due to --atomic flag"
    fi
    exit 1
fi

################################################################################
# Post-upgrade Verification
################################################################################
if [ "$DRY_RUN" = false ]; then
    print_header "Post-upgrade Verification"

    # Show new release status
    echo ""
    print_info "Release Status:"
    helm status "$RELEASE_NAME" -n "$NAMESPACE"

    # Get new revision
    NEW_REVISION=$(helm list -n "$NAMESPACE" -o json | jq -r ".[] | select(.name==\"$RELEASE_NAME\") | .revision")
    print_success "Upgraded from revision $CURRENT_REVISION to $NEW_REVISION"

    # Check deployment rollout status
    echo ""
    print_info "Checking deployment rollout status..."
    if kubectl rollout status deployment -n "$NAMESPACE" -l "app.kubernetes.io/instance=$RELEASE_NAME" --timeout=60s; then
        print_success "Deployment rollout successful"
    else
        print_error "Deployment rollout failed"
        print_info "Rolling back to revision $CURRENT_REVISION..."
        helm rollback "$RELEASE_NAME" "$CURRENT_REVISION" -n "$NAMESPACE"
        exit 1
    fi

    # Show deployed resources
    echo ""
    print_info "Deployed Resources:"
    kubectl get all -n "$NAMESPACE" -l "app.kubernetes.io/instance=$RELEASE_NAME"

    echo ""
    print_info "Useful Commands:"
    echo ""
    echo "  # View upgrade history"
    echo "  helm history $RELEASE_NAME -n $NAMESPACE"
    echo ""
    echo "  # Rollback to previous revision"
    echo "  helm rollback $RELEASE_NAME $CURRENT_REVISION -n $NAMESPACE"
    echo ""
    echo "  # Check pod status"
    echo "  kubectl get pods -n $NAMESPACE -l app.kubernetes.io/instance=$RELEASE_NAME"
    echo ""
    echo "  # View logs"
    echo "  kubectl logs -n $NAMESPACE -l app.kubernetes.io/instance=$RELEASE_NAME --tail=100 -f"
    echo ""

    print_success "Upgrade complete!"
else
    print_success "Dry-run complete!"
fi
